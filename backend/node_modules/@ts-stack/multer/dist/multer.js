...
export class Multer {
  ...
  ```
    ...
    parseLimit(limits, key, defaultValue) {
        const input = limits[key] ?? defaultValue;
        const value = bytes.parse(input);
        if (!Number.isFinite(value))
            throw new Error(`Invalid limit "${key}" given: ${limits[key]}`);
        if (!Number.isInteger(value))
            throw new Error(`Invalid limit "${key}" given: ${value}`);
        return value;
    }
    handle(limits, groups, fileStrategy, withoutGuard) {
        if (withoutGuard) {
